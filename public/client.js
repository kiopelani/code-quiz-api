$(function(){
  ...
  $('form').on('submit', function(event){
    ...
    $.ajax({
      type: 'POST', url: '/questions', data: questionData
    })
    .error(function(){
  ...
  $('.question-list').on('click', 'a[data-question]', function(event){
    ...
    $.ajax({
      type: 'DELETE', url: '/questions/' + target.data('question')
    }).done(function(){
  ...
  function appendToList(questions){
    ...
    for(var i in questions){
      ...
      content = '<a href="/questions/'+question+'">'+question+'</a>'+
